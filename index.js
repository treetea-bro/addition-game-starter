...
const App = {
  ...
  submitAnswer: async function () {
    ...
    if (answer !== result) alert("틀리셨습니다! 다시 한번 시도해보세요.");
    else {
      if ((await this.callContractBalance()) >= 0.1) {
        alert("정답입니다! 0.1 KLAY를 지급 해 드리겠습니다.");
        this.receiveKlay();
      } else {
        alert("죄송합니다. 컨트랙의 KLAY가 전부 소진되었습니다.");
      }
    }
  },
  ...
  changeUI: async function (walletInstance) {
    ...
    $("#contractBalance").append(
      `<p>이벤트 잔액: ${cav.utils.fromPeb(
        await this.callContractBalance(),
        "KLAY"
      )} KLAY</p>`
    );

    if (
      (await this.callOwner()).toUpperCase() ===
      walletInstance.address.toUpperCase()
    ) {
      $("#owner").show();
    }
  },
    ...
    agContract.methods
      ...
      .then((receipt) => {
        if (receipt.status) {
          spinner.stop();
          alert(
            `0.1 KLAY가 ${walletInstance.address} 계정으로 지급되었습니다.`
          );
          $("#transaction").html("");
          $("#transaction").append(
            `<p><a href="https://baobab.klaytnscope.com/tx/${receipt.transactionHash}" target=_blank>클레이튼 Scope에서 트랜젝션 확인</a></p>`
          );
          return agContract.methods
            .getBalance()
            .call()
            .then(async (balance) => {
